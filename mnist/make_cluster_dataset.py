    ...
    with torch.no_grad():
        for data, target in dataloader:
            ...
            for feature, label in zip(h, target):
                if label.item() in class_samples_count and class_samples_count[label.item()] < num_samples_per_class:
...
classes             = (0, 1, 2, 3)  # classes to save
...
train_loader, _ = vision_dataset.get_dataloaders(batch_size=batch_size, num_workers=num_workers)
